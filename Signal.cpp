...
void Signal::Initialize(Model* model_, Model* bodyModel, Model* taiyaModel)
{
	assert(model_);
	assert(bodyModel);
	assert(taiyaModel);
...
void Signal::Draw(ViewProjection viewProjection)
{
...
bool Signal::GetCheckUpdate()
{
	return isCheckPointUpdate;
}

void Signal::LoadTexture(TextureHandle tex)
{
	object::LoadTexture(tex);
}

void Signal::OnCollision()
{
